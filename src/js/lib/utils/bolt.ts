  ...
  return new Promise(function (resolve, reject) {
    ...
    csi.evalScript(
      `try{
          ...
          host["__serialize"](res);
        }catch(e){
          var _fn = (e.fileName && typeof File !== 'undefined') ? new File(e.fileName).fsName : (e.fileName || '');
          var _msg = String(e.message || e).replace(/\\\\/g,'\\\\\\\\').replace(/"/g,'\\\\"');
          var _name = String(e.name || 'Error').replace(/"/g,'\\\\"');
          '{"name":"' + _name + '","message":"' + _msg + '","fileName":"' + _fn + '","line":' + (e.line || 0) + '}';
        }`,
      (res: string) => {
        try {
          //@ts-ignore
...
const fileExists = (file: string): boolean => {
  ...
  if (!cepFs || typeof cepFs.stat !== "function") return false;

  const result = cepFs.stat(file);
  return result.err === 0 && result.data.isFile();
};
...
export const initBolt = (log = true) => {
  if (window.cep) {
    ...
    if (fileExists(jsxSrc)) {
      if (log) console.log(jsxSrc);
      evalFile(jsxSrc);
    } else if (fileExists(jsxBinSrc)) {
